...
if ($null -eq $uninstallKeys -and $localVerArray.length -gt 0) {
    ...
    foreach ($string in $uninstallKeys) {
        if ($string -match "msiexec.exe") {
            ...
            $code = (Start-Process "C:\Windows\System32\msiexec.exe" -ArgumentList $msiArgs -Wait -PassThru).ExitCode
            $sw.Stop()
            $actionEnd = Get-Date
        ...
        elseif ($string.QuietUninstallString) {
            ...
            $actionStart = Get-Date
            $sw = [System.Diagnostics.Stopwatch]::StartNew()
            & cmd /c $string.QuietUninstallString
...
Write-ProcessSnapshot "After Uninstall"

# --- Pre-Install Process Kill ---
# After uninstall, background services (e.g. Google Update) can respawn and
# start their own MSI transactions, grabbing the _MSIExecute mutex. Kill
# target processes again so the install MSI gets a clean mutex.
...
Write-ProcessSnapshot "Pre-Install"
...
try {
    foreach ($line in $installLineArray) {
        ...
        # --- Retry on 1603 (MSI custom action failure) ---
        # After an uninstall, the MSI service may still be finalizing cleanup.
        # If the install hits 1603, wait and retry once.
        if ($installExit -eq 1603) {
            Start-Sleep -Seconds 10
