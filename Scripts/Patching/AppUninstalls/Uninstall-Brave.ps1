...
# Brave-specific process kills
$braveProcs = @('brave', 'BraveUpdate', 'BraveCrashHandler', 'BraveCrashHandler64')
foreach ($proc in $braveProcs) {
...
# =============================================================================
# Brave Browser is a per-user Chromium install. It places files under each
# user's AppData\Local\BraveSoftware and registers uninstall keys under HKU
# SIDs rather than HKLM. Cleanup must enumerate all loaded user hives.
# =============================================================================


# --- Per-user uninstall via HKU registry ---
# The standard uninstall section above only checks HKLM/HKCU. Brave registers
# its uninstaller under each user's HKU SID. Walk all loaded hives and invoke
# any QuietUninstallString or UninstallString found.
...
$sidPaths = @(Get-ChildItem "HKU:\" -ErrorAction 0 |
    Where-Object { $_.Name -match 'S-1-5-21-' -and $_.Name -notmatch '_Classes$' })

$hkuUninstallCount = 0

foreach ($sidEntry in $sidPaths) {
    $sid = $sidEntry.PSChildName
    $hkuUninstallKey = "HKU:\$sid\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\BraveSoftware Brave-Browser"

    if (-not (Test-Path $hkuUninstallKey -ErrorAction 0)) { continue }

    $braveReg = Get-ItemProperty $hkuUninstallKey -ErrorAction 0

    if ($braveReg.QuietUninstallString) {
        & cmd /c $braveReg.QuietUninstallString
        $hkuUninstallCount++
    }
    elseif ($braveReg.UninstallString) {
        & cmd /c $braveReg.UninstallString
        $hkuUninstallCount++
    }
}
...
if ($hkuUninstallCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Uninstall'
        Command   = "HKU per-user uninstall ($hkuUninstallCount users)"
        ExitCode  = $LASTEXITCODE
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Brave-HKU-Uninstall'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# The per-user uninstaller may leave processes running. Kill everything again
# before registry and file cleanup.
...
# --- Stop and remove Brave services ---

$braveServices = @(Get-Service -ErrorAction 0 | Where-Object {
    $_.Name -match '^(brave|BraveUpdate|BraveElevation)'
})

foreach ($svc in $braveServices) {
...
$braveTasks = @(Get-ScheduledTask -ErrorAction 0 | Where-Object {
    $_.TaskName -match 'Brave(Update|Software)'
})
...
# --- Registry cleanup ---
# Remove Brave and BraveSoftware registry keys from all loaded HKU hives,
# HKLM, and HKCR.
...
# HKLM and HKCR keys that may reference Brave
$braveRegKeys = @(
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
...
$sidRegTemplates = @(
    'HKU:\USID\SOFTWARE\BraveSoftware'
    'HKU:\USID\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\BraveSoftware Brave-Browser'
    'HKU:\USID_Classes\BraveSHTML'
    'HKU:\USID_Classes\BraveSSHTML'
    'HKU:\USID_Classes\BraveHTML'
    'HKU:\USID_Classes\.shtml\OpenWithProgids'
)
...
# Remove Brave from RegisteredApplications if present
...
if (Test-Path $regAppsPath -ErrorAction 0) {
    ...
    if ($regApps.PSObject.Properties.Name -contains 'Brave') {
        Remove-ItemProperty $regAppsPath -Name 'Brave' -Force -ErrorAction SilentlyContinue
        $regDeleteCount++
    }
}

# Remove Brave from StartMenuInternet
$startMenuPath = 'HKLM:\SOFTWARE\Clients\StartMenuInternet\Brave'
...
# Brave installs entirely under per-user AppData. Enumerate all user profiles
# and target the BraveSoftware directory tree.
...
foreach ($u in $userArray2) {
    $perUserFolders += "C:\Users\$u\AppData\Local\BraveSoftware"
}

$allFolders = @($perUserFolders)
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "ACL ownership and FullControl on BraveSoftware folders"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'Brave-ACL'
    StartTime = $actionStart
...
# --- Delete Brave files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount BraveSoftware folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Brave-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Brave-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $braveProcs
