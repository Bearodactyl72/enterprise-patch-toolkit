...
# Bridge-specific processes only
...
# =============================================================================
# Bridge-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Bridge only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically.
# =============================================================================


# --- Kill Bridge processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove Bridge-specific registry keys only. Scoped to $softwareName to
...
$brRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Bridge only
    'HKLM:\Software\Adobe'
...
# Clean up Bridge-specific vendor subkeys directly
# Covers all Bridge versions dynamically
$brVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'Bridge' })
    foreach ($sk in $subkeys) {
        $brVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $brVendorPaths) {
...
# --- Dynamic Bridge folder detection ---
# Find all "Bridge*" or "Adobe Bridge*" folders under Adobe in both
# Program Files locations. Catches all Bridge versions dynamically.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'Bridge' })
        foreach ($f in $found) {
            $brFolders += $f.FullName
        }
    }
}
...
if ($brFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Bridge folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Bridge-ACL'
        StartTime = $actionStart
...
# --- Kill Bridge processes one final time before deletion ---
...
# --- Delete Bridge files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Bridge folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Bridge-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Bridge-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $bridgeProcs
