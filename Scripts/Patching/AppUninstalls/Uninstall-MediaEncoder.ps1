...
# Media Encoder-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$meProcs = @('Adobe Media Encoder')
foreach ($proc in $meProcs) {
...
# =============================================================================
# Media Encoder-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Media Encoder only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically.
# =============================================================================


# --- Kill Media Encoder processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove Media Encoder-specific registry keys only. Scoped to $softwareName to
...
$meRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Media Encoder only
    'HKLM:\Software\Adobe'
...
# Clean up Media Encoder-specific vendor subkeys directly
# Covers all versions dynamically
$meVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'Media Encoder' })
    foreach ($sk in $subkeys) {
        $meVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $meVendorPaths) {
...
# --- Dynamic Media Encoder folder detection ---
# Find all "Media Encoder*" or "Adobe Media Encoder*" folders under Adobe in both
# Program Files locations. Catches all versions dynamically.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'Media Encoder' })
        foreach ($f in $found) {
            $meFolders += $f.FullName
        }
    }
}
...
if ($meFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Media Encoder folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'MediaEncoder-ACL'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# --- Kill Media Encoder processes one final time before deletion ---
...
# --- Delete Media Encoder files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Media Encoder folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'MediaEncoder-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Media Encoder-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $meProcs
