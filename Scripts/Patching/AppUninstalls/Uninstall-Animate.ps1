...
# Animate-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$animateProcs = @('Animate')
foreach ($proc in $animateProcs) {
...
# =============================================================================
# Animate-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Animate only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically.
# =============================================================================


# --- Kill Animate processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove Animate-specific registry keys only. Scoped to $softwareName to
...
$anRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Animate only
    'HKLM:\Software\Adobe'
...
# Clean up Animate-specific vendor subkeys directly
# Covers all Animate versions dynamically
$anVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'Animate' })
    foreach ($sk in $subkeys) {
        $anVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $anVendorPaths) {
...
# --- Dynamic Animate folder detection ---
# Find all "Animate*" or "Adobe Animate*" folders under Adobe in both
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'Animate' })
        foreach ($f in $found) {
            $anFolders += $f.FullName
        }
    }
}
...
if ($anFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Animate folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Animate-ACL'
        StartTime = $actionStart
...
# --- Kill Animate processes one final time before deletion ---
...
# --- Delete Animate files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Animate folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Animate-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Animate-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $animateProcs
