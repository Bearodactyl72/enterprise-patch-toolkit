...
# --- Chrome setup.exe uninstall ---
# Modern Chrome uses an EXE-based installer, not MSI. The registry-based
# uninstall above may find nothing. Run setup.exe --uninstall directly.

$setupPaths = @(
    Get-ChildItem "C:\Program Files\Google\Chrome\Application\*\Installer\setup.exe" -ErrorAction 0
    Get-ChildItem "C:\Program Files (x86)\Google\Chrome\Application\*\Installer\setup.exe" -ErrorAction 0
)

foreach ($setupExe in $setupPaths) {
    if ($null -ne $setupExe) {
        $setupArgs = "--uninstall --force-uninstall --system-level --channel=stable"
        $actionStart = Get-Date
        $sw = [System.Diagnostics.Stopwatch]::StartNew()
        $code = (Start-Process $setupExe.FullName -ArgumentList $setupArgs -Wait -PassThru).ExitCode
        $sw.Stop()
        $actionEnd = Get-Date
...
# These services lock files and can block uninstall/reinstall.
# Includes legacy (gupdate/gupdatem) and new-style (GoogleUpdater*) services.

$googleServices = @(Get-Service -ErrorAction 0 | Where-Object {
    $_.Name -match '^(gupdate|gupdatem|GoogleChromeElevation|GoogleUpdater)'
})
...
$googleTasks = @(Get-ScheduledTask -ErrorAction 0 | Where-Object {
    $_.TaskName -match 'Google(Update|Updater)'
})

foreach ($task in $googleTasks) {
...
# Google services and scheduled tasks can respawn processes. Kill everything
# again before we start deleting files and registry keys.

foreach ($process in $processName -split " ") {
    Get-Process $process -ErrorAction 0 | Stop-Process -Force
}
# Catch Chrome-specific processes not in config's processName
$chromeProcs = @('chrome_proxy', 'setup', 'elevation_service', 'software_reporter_tool')
...
# --- Registry cleanup ---
# Remove Chrome and Google Update registry keys that can cause the installer
# to think Chrome is already installed or that an install is in progress.
...
foreach ($regKey in $chromeRegKeys) {
    ...
    $matchingKeys = @(Get-ItemProperty $children.PSPath -ErrorAction 0 |
        Where-Object { $_ -match 'Chrome|Google' })
...
# Clean up direct vendor key paths (entire subtree if they exist)
$vendorPaths = @(
    ...
    'HKCU:\Software\Google\Software Removal Tool'
)
...
# Remove Chrome from StartMenuInternet and RegisteredApplications property
$startMenuPath = 'HKLM:\SOFTWARE\Clients\StartMenuInternet\Google Chrome'
...
if (Test-Path $regAppsPath -ErrorAction 0) {
    ...
    if ($regApps.PSObject.Properties.Name -contains 'Google Chrome') {
        Remove-ItemProperty $regAppsPath -Name 'Google Chrome' -Force -ErrorAction SilentlyContinue
        $regDeleteCount++
    }
}
...
# Google's installer sets restrictive ACLs on its folders. Take ownership
...
# Per-user folders (Application, Update, CrashReports, Software Reporter)
# Explicitly excludes User Data (bookmarks, history, profiles)
...
foreach ($u in $userArray2) {
    $perUserFolders += "C:\Users\$u\AppData\Local\Google\Chrome\Application"
    $perUserFolders += "C:\Users\$u\AppData\Local\Google\Update"
    $perUserFolders += "C:\Users\$u\AppData\Local\Google\CrashReports"
    $perUserFolders += "C:\Users\$u\AppData\Local\Google\Software Reporter Tool"
}
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "ACL ownership and FullControl on Google folders"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'Chrome-ACL'
    StartTime = $actionStart
...
# Chrome processes can respawn even after service deletion. Final sweep.
...
# --- Delete Chrome files and folders ---
...
# Clean up empty parent Google folders
$parentFolders = @(
    'C:\Program Files\Google'
    'C:\Program Files (x86)\Google'
)
...
# =============================================================================
# End Chrome-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $chromeProcs
