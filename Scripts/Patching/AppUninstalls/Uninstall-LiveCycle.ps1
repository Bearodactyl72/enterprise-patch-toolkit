...
# LiveCycle-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$lcProcs = @('LiveCycle', 'Forms', 'FormDesigner')
foreach ($proc in $lcProcs) {
...
# =============================================================================
# LiveCycle-Specific Cleanup
# Also known as Forms Designer / Designer ES4
# =============================================================================
# Surgical removal of Adobe LiveCycle Designer only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically.
# =============================================================================


# --- Kill LiveCycle processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove LiveCycle-specific registry keys only. Scoped to $softwareName to
...
$lcRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to LiveCycle only
    'HKLM:\Software\Adobe'
...
# Clean up LiveCycle-specific vendor subkeys directly
# Covers all LiveCycle Designer versions
$lcVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'LiveCycle' })
    foreach ($sk in $subkeys) {
        $lcVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $lcVendorPaths) {
...
# --- Dynamic LiveCycle folder detection ---
# Find all "LiveCycle*" or "Adobe LiveCycle*" folders under Adobe in both
# Program Files locations. Catches all LiveCycle Designer versions.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'LiveCycle' })
        foreach ($f in $found) {
            $lcFolders += $f.FullName
        }
    }
}
...
if ($lcFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on LiveCycle folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'LiveCycle-ACL'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# --- Kill LiveCycle processes one final time before deletion ---
...
# --- Delete LiveCycle files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount LiveCycle folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'LiveCycle-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End LiveCycle-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $lcProcs
