...
# =============================================================================
# Intel PROSet/Wireless WiFi Software - Specific Cleanup
# =============================================================================
# Uses Get-Package + pnputil driver deletion with hardcoded .inf list.
# The .inf list may not still be valid -- targeting will be revisited if
# this software resurfaces as an issue.
# =============================================================================


# --- Package removal via Get-Package ---

$actionStart = Get-Date
$sw = [System.Diagnostics.Stopwatch]::StartNew()

try {
    Get-Package -Name "Intel*PROSet/Wireless WiFi Software" -ErrorAction Stop |
        Uninstall-Package -Force -ErrorAction Stop
    $packageExit = 0
}
catch {
    $packageExit = 1
    $regComment = "Package not found or removal failed"
}
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Uninstall'
    Command   = 'Get-Package "Intel*PROSet/Wireless WiFi Software" | Uninstall-Package -Force'
    ExitCode  = $packageExit
    Duration  = $sw.Elapsed.ToString()
    Source    = 'Intel-GetPackage'
...
# --- Driver removal via pnputil ---
# Hardcoded .inf list -- may need updating if this software resurfaces
...
foreach ($inf in $driverInfs) {
    ...
    pnputil /delete-driver $inf /uninstall /force 2>$null
    ...
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Uninstall'
        Command   = "pnputil /delete-driver $inf /uninstall /force"
        ExitCode  = $driverExit
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Intel-PnpUtil'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Intel PROSet-Specific Cleanup
# =============================================================================
