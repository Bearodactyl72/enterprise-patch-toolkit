...
# FrameMaker-specific processes only
# Does NOT kill Office processes (Word, Excel, etc.) -- no dependency confirmed
# Does NOT kill Acrobat.exe or AcroRd32.exe -- separate products
$fmProcs = @('FrameMaker', 'FM')

Write-ProcessSnapshot "Before Process Kill" -Extra $fmProcs

foreach ($process in $processName -split " ") {
    Get-Process $process -ErrorAction 0 | Stop-Process -Force
}

foreach ($proc in $fmProcs) {
    Get-Process $proc -ErrorAction 0 | Stop-Process -Force
}

Write-ProcessSnapshot "After Process Kill" -Extra $fmProcs
...
# =============================================================================
# FrameMaker-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe FrameMaker only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets 2019, 2020, 2022, and future versions dynamically.
# =============================================================================


# --- Kill FrameMaker processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove FrameMaker-specific registry keys only. Scoped to $softwareName to
# avoid touching other Adobe products.
...
$fmRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to FrameMaker only
    'HKLM:\Software\Adobe'
...
# Clean up FrameMaker-specific vendor subkeys directly
# Covers 2019, 2020, 2022, and future versions
$fmVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'FrameMaker' })
    foreach ($sk in $subkeys) {
        $fmVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $fmVendorPaths) {
...
# --- Dynamic FrameMaker folder detection ---
# Find all "FrameMaker*" or "Adobe FrameMaker*" folders under Adobe in both
# Program Files locations. Catches 2019, 2020, 2022, and future versions.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'FrameMaker' })
        foreach ($f in $found) {
            $fmFolders += $f.FullName
        }
    }
}
...
if ($fmFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on FrameMaker folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'FrameMaker-ACL'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# --- Kill FrameMaker processes one final time before deletion ---
...
# --- Delete FrameMaker files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount FrameMaker folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'FrameMaker-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End FrameMaker-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $fmProcs
