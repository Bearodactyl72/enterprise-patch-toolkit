...
# =============================================================================
# FireEye-Specific Cleanup
# =============================================================================
# FireEye endpoint agent (xagt). Simple cleanup -- system-level install only.
...
foreach ($regKey in $fireeyeRegKeys) {
...
# FireEye installs to Program Files (x86) only
...
foreach ($folder in $fireeyeFolders) {
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "ACL ownership and FullControl on FireEye folders"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'FireEye-ACL'
    StartTime = $actionStart
...
# --- Delete FireEye files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount FireEye folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'FireEye-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
