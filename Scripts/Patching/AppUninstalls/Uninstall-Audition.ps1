...
# Audition-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$auProcs = @('Adobe Audition', 'AdobeIPCBroker')
foreach ($proc in $auProcs) {
...
# =============================================================================
# Audition-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Audition only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically (CC 2019, 2020, 2022, 2024, etc.).
# =============================================================================


# --- Kill Audition processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove Audition-specific registry keys only. Scoped to $softwareName to
...
$auRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Audition only
    'HKLM:\Software\Adobe'
...
# Clean up Audition-specific vendor subkeys directly
# Covers CC 2019, 2020, 2022, 2024, and future versions
$auVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'Audition' })
    foreach ($sk in $subkeys) {
        $auVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $auVendorPaths) {
...
# --- Dynamic Audition folder detection ---
# Find all "Adobe Audition*" folders under Adobe in both Program Files
# locations. Catches CC 2019, 2020, 2022, 2024, and future versions.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'Audition' })
        foreach ($f in $found) {
            $auFolders += $f.FullName
        }
    }
}
...
if ($auFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Audition folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Audition-ACL'
        StartTime = $actionStart
...
# --- Kill Audition processes one final time before deletion ---
...
# --- Delete Audition files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Audition folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Audition-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Audition-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $auProcs
