...
# Lightroom-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$lrProcs = @('Lightroom', 'Adobe Lightroom', 'LogTransport2')
foreach ($proc in $lrProcs) {
...
# =============================================================================
# Lightroom-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Lightroom Classic only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically (Classic CC, Classic 2019+).
# =============================================================================


# --- Kill Lightroom processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove Lightroom-specific registry keys only. Scoped to $softwareName to
...
$lrRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Lightroom only
    'HKLM:\Software\Adobe'
...
# Clean up Lightroom-specific vendor subkeys directly
# Covers Classic CC, 2019, 2020, 2022, and future versions
$lrVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'Lightroom' })
    foreach ($sk in $subkeys) {
        $lrVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $lrVendorPaths) {
...
# --- Dynamic Lightroom folder detection ---
# Find all "Lightroom*" or "Adobe Lightroom*" folders under Adobe in both
# Program Files locations. Catches Classic CC, Classic 2019+, and future versions.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'Lightroom' })
        foreach ($f in $found) {
            $lrFolders += $f.FullName
        }
    }
}
...
if ($lrFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Lightroom folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Lightroom-ACL'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# --- Kill Lightroom processes one final time before deletion ---
...
# --- Delete Lightroom files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Lightroom folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Lightroom-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Lightroom-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $lrProcs
