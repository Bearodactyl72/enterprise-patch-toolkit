...
# After Effects-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$aeProcs = @('AfterFX', 'Adobe After Effects')
foreach ($proc in $aeProcs) {
...
# =============================================================================
# After Effects-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe After Effects only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically (CC, 2019+).
# =============================================================================


# --- Kill After Effects processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove After Effects-specific registry keys only. Scoped to $softwareName to
...
$aeRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to After Effects only
    'HKLM:\Software\Adobe'
...
# Clean up After Effects-specific vendor subkeys directly
# Covers CC, 2019, 2020, 2022, and future versions
$aeVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'After Effects' })
    foreach ($sk in $subkeys) {
        $aeVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $aeVendorPaths) {
...
# --- Dynamic After Effects folder detection ---
# Find all "After Effects*" or "Adobe After Effects*" folders under Adobe in both
# Program Files locations. Catches CC, 2019+, and future versions.
...
if ($aeFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on After Effects folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'AfterEffects-ACL'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# --- Kill After Effects processes one final time before deletion ---
...
# --- Delete After Effects files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount After Effects folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'AfterEffects-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End After Effects-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $aeProcs
