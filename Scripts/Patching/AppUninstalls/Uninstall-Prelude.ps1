...
# Prelude-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$preludeProcs = @('Adobe Prelude')
foreach ($proc in $preludeProcs) {
...
# =============================================================================
# Prelude-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Prelude only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically.
# =============================================================================


# --- Kill Prelude processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove Prelude-specific registry keys only. Scoped to $softwareName to
...
$lrRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Prelude only
    'HKLM:\Software\Adobe'
...
# Clean up Prelude-specific vendor subkeys directly
# Covers all Prelude versions
$lrVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'Prelude' })
...
# --- Dynamic Prelude folder detection ---
# Find all "Prelude*" or "Adobe Prelude*" folders under Adobe in both
# Program Files locations. Catches all Prelude versions.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'Prelude' })
        foreach ($f in $found) {
...
if ($lrFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Prelude folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Prelude-ACL'
        StartTime = $actionStart
...
# --- Kill Prelude processes one final time before deletion ---
...
# --- Delete Prelude files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Prelude folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Prelude-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Prelude-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $preludeProcs
