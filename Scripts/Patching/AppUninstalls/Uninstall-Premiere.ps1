...
# Premiere Pro-specific processes only
# Does NOT kill Acrobat, Reader, or other Adobe products
$premProcs = @('Adobe Premiere Pro')
foreach ($proc in $premProcs) {
...
# =============================================================================
# Premiere Pro-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Premiere Pro only.
# Does NOT touch Acrobat, Reader, or other Adobe products.
# Targets all versions dynamically (CC, 2019+).
# =============================================================================


# --- Kill Premiere Pro processes again after uninstall ---
...
# --- Registry cleanup ---
# Remove Premiere Pro-specific registry keys only. Scoped to $softwareName to
...
$premRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Premiere Pro only
    'HKLM:\Software\Adobe'
...
# Clean up Premiere Pro-specific vendor subkeys directly
...
foreach ($adobeRoot in @(
    ...
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match 'Premiere' })
    foreach ($sk in $subkeys) {
        $premVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $premVendorPaths) {
...
# --- Dynamic Premiere Pro folder detection ---
# Find all "Premiere Pro*" or "Adobe Premiere Pro*" folders under Adobe in both
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match 'Premiere Pro' })
        foreach ($f in $found) {
            $premFolders += $f.FullName
        }
    }
}
...
if ($premFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Premiere Pro folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Premiere-ACL'
        StartTime = $actionStart
...
# --- Kill Premiere Pro processes one final time before deletion ---
...
# --- Delete Premiere Pro files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Premiere Pro folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Premiere-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Premiere Pro-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $premProcs
