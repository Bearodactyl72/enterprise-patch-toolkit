...
# Edge-specific process kills
$edgeProcs = @('msedge', 'MicrosoftEdgeUpdate', 'identity_helper', 'msedge_proxy',
    'elevation_service', 'MicrosoftEdge_X*', 'cookie_exporter')
...
$uninstallKeys = Get-ItemProperty (Get-ChildItem $regKeys -ErrorAction SilentlyContinue -Force).PSPath |
    Where-Object {$_ -match "$softwareName" -and $_ -notmatch 'WebView'}
...
# =============================================================================
# Microsoft Edge is a system-level Chromium browser. It registers as a Windows
# component and resists standard uninstall. This section uses setup.exe with
# --force-uninstall, removes Edge Update services/tasks, cleans registry, and
# deletes leftover files. WebView2 is intentionally preserved -- many apps and
# Windows components depend on it.
# =============================================================================


# --- Edge setup.exe uninstall ---
# Edge uses an EXE-based installer like Chrome. The registry uninstall above
# may not fully remove it, especially the system-bundled copy. Run setup.exe
# directly with --force-uninstall to bypass the Windows component protection.

$setupPaths = @(
    Get-ChildItem "C:\Program Files (x86)\Microsoft\Edge\Application\*\Installer\setup.exe" -ErrorAction 0
    Get-ChildItem "C:\Program Files\Microsoft\Edge\Application\*\Installer\setup.exe" -ErrorAction 0
)

foreach ($setupExe in $setupPaths) {
    if ($null -ne $setupExe) {
        $setupArgs = "--uninstall --system-level --force-uninstall"
        $actionStart = Get-Date
        $sw = [System.Diagnostics.Stopwatch]::StartNew()
        $proc = Start-Process $setupExe.FullName -ArgumentList $setupArgs -Wait -PassThru
        $code = $proc.ExitCode
        $sw.Stop()
        $actionEnd = Get-Date
        ...
        $actionEntry = [PSCustomObject]@{
            Phase     = 'Uninstall'
            Command   = "$($setupExe.FullName) $setupArgs"
            ExitCode  = $code
            Duration  = $sw.Elapsed.ToString()
            Source    = 'Edge-SetupExe'
            StartTime = $actionStart
            EndTime   = $actionEnd
        }
        $actionLog.Add($actionEntry) > $null
...
# --- Remove Edge Appx/MSIX packages ---
# Newer Windows versions may deliver Edge as a provisioned Appx package.
# Remove both per-user and provisioned packages to prevent auto-reinstall
# before our clean install runs. Exclude WebView2 packages.
...
$edgeAppx = @(Get-AppxPackage -AllUsers -ErrorAction 0 |
    Where-Object { $_.Name -match 'MicrosoftEdge' -and $_.Name -notmatch 'WebView' })

# DISM/AppX cmdlets can throw terminating COMExceptions that bypass
# -ErrorAction SilentlyContinue (e.g. "user was logged off" = 0x80073CF2
# when AppXSvc can't impersonate an active session from SYSTEM/WinRM).
# Wrap each call so one bad package doesn't unwind the whole script.
foreach ($pkg in $edgeAppx) {
    try {
        ...
        $appxFailed++
        $appxErrors += "Remove-AppxPackage $($pkg.Name): $($_.Exception.Message)"
    }
}

$edgeProvisioned = @(Get-AppxProvisionedPackage -Online -ErrorAction 0 |
    Where-Object { $_.DisplayName -match 'MicrosoftEdge' -and $_.DisplayName -notmatch 'WebView' })

foreach ($pkg in $edgeProvisioned) {
    try {
        ...
        $appxFailed++
        $appxErrors += "Remove-AppxProvisionedPackage $($pkg.PackageName): $($_.Exception.Message)"
    }
}
...
if (($appxRemoved + $appxFailed) -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Uninstall'
        Command   = "Remove Edge Appx/Provisioned packages ($appxRemoved removed, $appxFailed failed)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Edge-Appx'
        StartTime = $actionStart
    ...
    if ($appxErrors.Count -gt 0) {
        $actionEntry | Add-Member -NotePropertyName 'Error' -NotePropertyValue ($appxErrors -join ' | ')
    }
    $actionLog.Add($actionEntry) > $null
    Write-ActionToLog $actionEntry
}
...
# These services lock files and can reinstall Edge via Windows Update.

$edgeServices = @(Get-Service -ErrorAction 0 | Where-Object {
...
foreach ($svc in $edgeServices) {
    ...
    # Mark for deletion -- sc.exe delete removes the service registration
    $actionStart = Get-Date
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    ...
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "sc.exe delete $($svc.Name)"
        ExitCode  = $svcExit
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Edge-ServiceDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
$edgeTasks = @(Get-ScheduledTask -ErrorAction 0 | Where-Object {
    $_.TaskName -match 'MicrosoftEdgeUpdate'
})

foreach ($task in $edgeTasks) {
    $taskFullName = Join-Path $task.TaskPath $task.TaskName
    $actionStart = Get-Date
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    ...
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "Unregister-ScheduledTask $taskFullName"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Edge-ScheduledTask'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# Edge services and scheduled tasks can respawn processes. Kill everything
# again before we start deleting files and registry keys.
...
# --- Registry cleanup ---
# Remove Edge and Edge Update registry keys that can cause the installer
# to think Edge is already installed or that an install is in progress.
# IMPORTANT: Do NOT touch EdgeWebView or WebView2 registry keys.
...
foreach ($regKey in $edgeRegKeys) {
    ...
    # For container keys (Uninstall hives, Installer\Products), only remove
    # child keys that match Edge but NOT WebView
    $isContainerKey = $regKey -match '\\Uninstall$' -or $regKey -match '\\Products$'
    ...
    if ($isContainerKey) {
        ...
        $matchingKeys = @(Get-ItemProperty $children.PSPath -ErrorAction 0 |
            Where-Object { $_ -match 'Microsoft Edge' -and $_ -notmatch 'WebView' })

        foreach ($key in $matchingKeys) {
            # Preserve RegisteredApplications -- other apps reference this
...
# Remove Edge from StartMenuInternet and RegisteredApplications property
$startMenuPath = 'HKLM:\SOFTWARE\Clients\StartMenuInternet\Microsoft Edge'
if (Test-Path $startMenuPath -ErrorAction 0) {
    Remove-Item $startMenuPath -Recurse -Force -ErrorAction SilentlyContinue
    $regDeleteCount++
}

$regAppsPath = 'HKLM:\SOFTWARE\RegisteredApplications'
if (Test-Path $regAppsPath -ErrorAction 0) {
    $regApps = Get-ItemProperty $regAppsPath -ErrorAction 0
    if ($regApps.PSObject.Properties.Name -contains 'Microsoft Edge') {
        Remove-ItemProperty $regAppsPath -Name 'Microsoft Edge' -Force -ErrorAction SilentlyContinue
        $regDeleteCount++
    }
}
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "Registry cleanup ($regDeleteCount keys/properties removed)"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
...
# Edge's installer sets restrictive ACLs on its folders. Take ownership
# and grant full control before attempting deletion.
# IMPORTANT: Do NOT touch EdgeWebView -- only target Edge browser folders.
...
# Per-user Edge folders (exclude WebView2 and User Data)
[System.Collections.ArrayList]$userArray2 = (Get-ChildItem "C:\Users" -ErrorAction 0).Name
$userArray2.Remove('Public') > $null
$userArray2.Remove('ADMINI~1') > $null

$perUserFolders = @()
foreach ($u in $userArray2) {
    $perUserFolders += "C:\Users\$u\AppData\Local\Microsoft\Edge"
}

$allFolders = @($systemFolders) + @($perUserFolders)

# Take ownership and grant FullControl on all target folders
$actionStart = Get-Date
$sw = [System.Diagnostics.Stopwatch]::StartNew()
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "ACL ownership and FullControl on Edge folders"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'Edge-ACL'
    StartTime = $actionStart
...
# --- Kill processes one final time before deletion ---
# Edge processes can respawn even after service deletion. Final sweep.
...
# Clean up empty parent Microsoft folder only if completely empty
# (other Microsoft products live here -- do not remove if anything remains)
$parentFolders = @(
    'C:\Program Files (x86)\Microsoft'
    'C:\Program Files\Microsoft'
)
foreach ($parent in $parentFolders) {
    if (Test-Path $parent -ErrorAction 0) {
        $remaining = @(Get-ChildItem $parent -Force -ErrorAction 0)
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "File/folder deletion ($deleteCount folders targeted)"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'Edge-FileDelete'
    StartTime = $actionStart
    EndTime   = $actionEnd
}
$actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Edge-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $edgeProcs
