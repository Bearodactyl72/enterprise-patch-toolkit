...
# WebEx-specific process kills
$webexProcs = @('WebEx', 'atmgr', 'CiscoWebExWebService', 'CiscoWebExStart')
foreach ($proc in $webexProcs) {
...
# =============================================================================
# WebEx-Specific Cleanup
# =============================================================================
# Cisco WebEx (post-rebrand of Spark). Cleans up both WebEx and legacy Spark
# launcher artifacts since WebEx inherited the Spark install structure.
# =============================================================================


# --- Registry cleanup ---
# WebEx stores per-user keys under HKU SIDs. Search all loaded SID hives
# for WebEx and Cisco vendor keys.
...
$sidRegTemplates = @(
    ...
    'HKU:\USID\SOFTWARE\Cisco Systems, Inc.'
)
...
foreach ($regKey in $webexRegKeys) {
...
# WebEx installs to system and per-user locations. Also clean up legacy
# Spark launcher paths that WebEx inherited during the rebrand.
...
# Per-user WebEx and legacy Spark launcher paths
$allUsers = @(Get-ChildItem "C:\Users" -Force -ErrorAction 0 |
    Where-Object { $_ -notmatch 'Public' })

$perUserFolders = @()
foreach ($u in $allUsers) {
    $perUserFolders += "C:\Users\$u\AppData\Roaming\WebEx"
    $perUserFolders += "C:\Users\$u\AppData\Local\WebEx"
    $perUserFolders += "C:\Users\$u\AppData\LocalLow\WebEx"
    $perUserFolders += "C:\Users\$u\AppData\Local\Programs\WebEx"
    # Legacy Spark launcher paths inherited by WebEx
    $perUserFolders += "C:\Users\$u\AppData\Local\CiscoSparkLauncher"
    $perUserFolders += "C:\Users\$u\AppData\Local\Programs\Cisco_Spark_archive"
}
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "ACL ownership and FullControl on WebEx folders"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'WebEx-ACL'
    StartTime = $actionStart
...
# --- Delete WebEx files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount WebEx folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'WebEx-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End WebEx-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $webexProcs
