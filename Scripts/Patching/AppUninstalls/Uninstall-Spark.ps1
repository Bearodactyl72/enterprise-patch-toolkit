...
# =============================================================================
# Spark-Specific Cleanup
# =============================================================================
# Cisco Spark (pre-rebrand name for Webex). If Spark is corrupted it cannot
# auto-update to Webex, so a clean removal is needed before reinstalling.
# =============================================================================


# --- Registry cleanup ---
# Spark stores per-user keys under HKU SIDs. Search all loaded SID hives
# for Spark and Cisco vendor keys.
...
foreach ($regKey in $sparkRegKeys) {
...
# Spark installs to system and per-user locations.
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "ACL ownership and FullControl on Spark folders"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'Spark-ACL'
    StartTime = $actionStart
...
# --- Delete Spark files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Spark folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Spark-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
