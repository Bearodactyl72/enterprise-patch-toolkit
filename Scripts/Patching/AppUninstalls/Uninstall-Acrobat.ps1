...
# Acrobat-specific processes -- do NOT kill AcroRd32.exe (Reader) or FrameMaker
$acrobatProcs = @(
    'Acrobat'
    'AcroCEF'
    'AcroBroker'
    'AdobeCollabSync'
    'AdobeARM'
    # Creative Cloud updater processes -- Acrobat Pro often ships via CC
    'CreativeCloudPackager'
    'PDapp'
    'AAM Updates Notifier'
)
foreach ($proc in $acrobatProcs) {
...
# =============================================================================
# Acrobat-Specific Cleanup
# =============================================================================
# Surgical removal of Adobe Acrobat (Pro/Standard) -- any version.
# Does NOT touch Adobe Acrobat Reader or FrameMaker.
# Targets DC, 2017, 2020, 2022, and future versions dynamically.
# =============================================================================


# --- Stop Adobe update service ---
# AdobeARMservice locks Acrobat files and can block uninstall/reinstall.
# We stop it but do NOT delete it -- if Reader is also installed, it still
# needs this service.
...
# --- Registry cleanup ---
# Remove Acrobat-specific registry keys only. Uses $softwareName filter to
# match whichever version is installed. Explicitly targets "Adobe\Acrobat"
# vendor subkey but NOT "Adobe\Acrobat Reader".
...
$acrobatRegKeys = @(
    ...
    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall'
    'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall'
    # Adobe vendor keys -- search children, filter to Acrobat (not Reader)
    'HKLM:\Software\Adobe'
...
foreach ($regKey in $acrobatRegKeys) {
...
# Clean up Acrobat-specific vendor subkeys directly
# Pattern: "Acrobat" but NOT "Acrobat Reader" -- covers DC, 2017, 2020, 2022
$acrobatVendorPaths = @()
foreach ($adobeRoot in @(
    ...
    if (-not (Test-Path $adobeRoot -ErrorAction 0)) { continue }
    $subkeys = @(Get-ChildItem $adobeRoot -ErrorAction 0 -Force |
        Where-Object { $_.PSChildName -match '^Acrobat' -and $_.PSChildName -notmatch 'Reader' })
    foreach ($sk in $subkeys) {
        $acrobatVendorPaths += $sk.PSPath
    }
}

foreach ($vPath in $acrobatVendorPaths) {
...
# --- Dynamic Acrobat folder detection ---
# Find all Acrobat folders (DC, 2017, 2020, 2022, etc.) under Adobe in both
# Program Files locations. Explicitly excludes "Acrobat Reader" folders.
...
foreach ($pfRoot in @('C:\Program Files\Adobe', 'C:\Program Files (x86)\Adobe')) {
    if (Test-Path $pfRoot -ErrorAction 0) {
        $found = @(Get-ChildItem $pfRoot -Directory -ErrorAction 0 |
            Where-Object { $_.Name -match '^Acrobat' -and $_.Name -notmatch 'Reader' })
        foreach ($f in $found) {
            $acrobatFolders += $f.FullName
        }
    }
}
...
foreach ($folder in $acrobatFolders) {
...
if ($acrobatFolders.Count -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "ACL ownership and FullControl on Acrobat folders"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Acrobat-ACL'
        StartTime = $actionStart
...
# --- Kill Acrobat processes one final time before deletion ---
...
# --- Delete Acrobat files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Acrobat folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Acrobat-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
...
# =============================================================================
# End Acrobat-Specific Cleanup
# =============================================================================



Write-ProcessSnapshot "After Cleanup" -Extra $acrobatProcs
