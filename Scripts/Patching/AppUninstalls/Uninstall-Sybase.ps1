...
# Sybase suite uses multiple product names in registry
$sybaseNames = @(
    'Sybase'
    'SPS'
)
        ...
        elseif ($string.UninstallString) {
            ...
            if ($null -eq $parts) {
                ...
                $actionEntry = [PSCustomObject]@{
                    ...
                    ExitCode  = $null
                    Duration  = '00:00:00'
                    Source    = "Registry-Uninstall ($name)"
...
if (-not $foundAnyKeys -and $localVerArray.length -gt 0) {
...
# =============================================================================
# Sybase-Specific Cleanup
# =============================================================================
# Sybase/SPS suite with associated Cognos and PD2 components.
# Custom vendor registry keys for American Management Systems, CACI, and Cognos.
# =============================================================================
...
# Remove custom vendor keys specific to the Sybase/SPS suite
$customVendorKeys = @(
    'HKLM:\Software\Wow6432Node\Wow6432Node\American Management Systems, Inc.'
    'HKLM:\Software\Wow6432Node\Wow6432Node\CACI, Inc'
    'HKLM:\Software\Wow6432Node\Wow6432Node\Cognos'
    'HKLM:\SOFTWARE\WOW6432Node\American Management Systems, Inc.'
    'HKLM:\SOFTWARE\WOW6432Node\CACI, Inc.'
    'HKLM:\SOFTWARE\WOW6432Node\Cognos'
)

foreach ($vKey in $customVendorKeys) {
    if (Test-Path $vKey -ErrorAction 0) {
        Remove-Item $vKey -Recurse -Force -ErrorAction SilentlyContinue
        $regDeleteCount++
    }
}

# Search standard registry keys for Sybase/SPS product entries
foreach ($name in $sybaseNames) {
    foreach ($regKey in $sybaseRegKeys) {
...
# Sybase suite installs to Cognos, PD2, and Sybase folders

$sybaseFolders = @(
    'C:\Program Files (x86)\Cognos'
    'C:\Program Files (x86)\PD2'
    'C:\Program Files (x86)\Sybase'
...
foreach ($folder in $sybaseFolders) {
...
$actionEntry = [PSCustomObject]@{
    Phase     = 'Cleanup'
    Command   = "ACL ownership and FullControl on Sybase/Cognos/PD2 folders"
    ExitCode  = 0
    Duration  = $sw.Elapsed.ToString()
    Source    = 'Sybase-ACL'
    StartTime = $actionStart
...
# --- Delete Sybase files and folders ---
...
if ($deleteCount -gt 0) {
    $actionEntry = [PSCustomObject]@{
        Phase     = 'Cleanup'
        Command   = "File/folder deletion ($deleteCount Sybase folders targeted)"
        ExitCode  = 0
        Duration  = $sw.Elapsed.ToString()
        Source    = 'Sybase-FileDelete'
        StartTime = $actionStart
        EndTime   = $actionEnd
    }
    $actionLog.Add($actionEntry) > $null
